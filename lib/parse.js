...
function styles(definition, el, options, cb) {
  var file = definition.parent.file,
...
function component(mod, opts, cb) {
  ...
  iterator(mod, elements, styles, opts, function (err) {
    if (err) {
      return cb(err);
    }
    ...
    iterator(mod, elements, scripts, opts, function (err) {
      if (err) {
        return cb(err);
      }
      ...
      iterator(mod, elements, templates, opts, function (err) {
        if (err) {
          return cb(err);
        }
...
function parse(input, cb) {
  modules(input, input.result.load.files, input.options || {}, cb);
}
