...
function trucks(opts, cb) {
  ...
  load(options, function (err, loaded) {
    if (err) {
      return cb(err);
    }

    parse(loaded, options, function (err, parsed) {
      if (err) {
        return cb(err);
      }
      transform(parsed, options, function (err, transformed) {
        if (err) {
          return cb(err);
        }
        generate(transformed, options, function (err, generated) {
          if (err) {
            return cb(err);
          }
          write(generated, options, function (err, written) {
            if (err) {
              return cb(err);
            }
            cb(null, written);
