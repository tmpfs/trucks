...
function render(t, body, name, arg) {
  name = name || 'render';
  arg = arg || 'elem';
  return t.functionDeclaration(t.identifier(name), [t.identifier(arg)], t.blockStatement(body));
}
    ...
    for (i = 0; i < childNodes.length; i++) {
      ...
      if (child.type === TAG) {
        ...
        if (child.childNodes && child.childNodes.length) {
          ...
          var func = t.arrowFunctionExpression([], t.blockStatement(block));
          args.push(func);
        }
        ...
        if (opts.literals && opts.literals.text) {
          arg = babel.transform('`' + _el.text() + '`').ast.program.body[0].expression;
        } else {
          arg = t.stringLiteral(_el.text());
        }
