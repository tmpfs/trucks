...
function read(group, parent, state, cb) {
  ...
  // been loaded
  var pth = state.input.absolute(file);
...
function sources(files, input, output, state, parent, cb) {
  ...
  input.each(files, function (file, next) {
    ...
    var group = new state.input.components.File(pth);
    group.href = file;
...
function plugin() /*state, conf*/{
  return function load(state, cb) {
    ...
    sources(state.files, state, state.result.files, null, function (err) {
      if (err) {
        return cb(err);
      }
