...
function component(mod, state, cb) {
  ...
  var elements = $(selectors.styles, context).toArray();
  iterator(mod, state, elements, styles, function (err) {
    if (err) {
      return cb(err);
    }
    ...
    iterator(mod, state, elements, scripts, function (err) {
      if (err) {
        return cb(err);
      }
      ...
      iterator(mod, state, elements, templates, function (err) {
        cb(err);
      });
    });
  });
